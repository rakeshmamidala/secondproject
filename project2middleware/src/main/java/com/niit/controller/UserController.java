...
public class UserController 
{
	...
	public ResponseEntity<?> registerUser(@RequestBody User user)
	{
		try
		{
			...
			{
				ErrorClazz error=new ErrorClazz(2,"username already exists..please choose another username");
				return new ResponseEntity<ErrorClazz>(error,HttpStatus.CONFLICT);
			}
			
			if(!userDao.isEmailValid(user.getEmail()))
			{
				ErrorClazz error=new ErrorClazz(3,"email already exists..please choose another email");
				return new ResponseEntity<ErrorClazz>(error,HttpStatus.CONFLICT);	
			}
		...
		{
			ErrorClazz error=new ErrorClazz(1,"unable to register userdatails");
	...
	public ResponseEntity<?> logout(HttpSession session)
	{
		...
		if(username==null)//with out login
		{
